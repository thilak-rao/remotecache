...
async function runInvalidStartup(
...
describe('startup validation e2e', () => {
  it('refuses to start when ADMIN_TOKEN is shorter than 16 characters', async () => {
    const { exitCode, stderr } = await runInvalidStartup({ ADMIN_TOKEN: 'short' });
  ...
  it('refuses to start on an unknown STORAGE_STRATEGY', async () => {
    const { exitCode, stderr } = await runInvalidStartup({ STORAGE_STRATEGY: 'azure' });
  ...
  it('refuses to start when gcs storage has no bucket', async () => {
    const { exitCode, stderr } = await runInvalidStartup({ STORAGE_STRATEGY: 'gcs' });
  ...
  it('refuses to start when eviction is configured with s3 storage', async () => {
    const { exitCode, stderr } = await runInvalidStartup({
      STORAGE_STRATEGY: 's3',
