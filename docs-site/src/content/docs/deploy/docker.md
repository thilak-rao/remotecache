...
The image declares a `HEALTHCHECK` that polls `GET /health` inside the container every 30 seconds, so `docker ps` and `docker inspect` report `healthy`/`unhealthy` without extra flags, and Compose `depends_on: condition: service_healthy` works out of the box. The built-in check follows `PORT` and `BIND_ADDRESS` and uses HTTPS when [direct TLS](#direct-tls) is configured. Wildcard binds are probed on loopback in the matching address family; a concrete interface address is probed directly. For direct TLS, the local probe does not validate the certificate because its loopback address may differ from the certificate hostname. The probe ignores `HTTP_PROXY` and `HTTPS_PROXY` (and their lowercase forms), so an outbound proxy inherited by the container cannot intercept it. Like the server, the probe runs as the unprivileged `bun` user, or as the user you start the container with. Override the check with `--health-cmd` to probe `/ready` instead, or disable it with `--no-healthcheck`.
