...
description: "Nx self-hosted caching went free to paid to free to deprecated. Here's why I run a free, MIT-licensed Nx remote cache I actually own — and what it adds."
head:
  - tag: title
...
- **May 21, 2026:** Nx [deprecated all four plugins](https://nx.dev/docs/reference/deprecated/self-hosted-cache-packages), citing **[CVE-2025-36852](/security/cve-2025-36852/)** (CREEP), a cache-poisoning vulnerability inherent to the single-credential design. Jeff Cross wrote: _"We published a CVE (CREEP CVE-2025-36852) last year against these packages to make it clear that they shouldn't be used for serious projects because of the inherent design flaw."_ The plugins remain on npm so existing builds don't break, but they receive no further updates or security fixes.
...
## What remotecache adds

`remotecache` speaks the Nx remote-cache HTTP contract — the exact endpoints Nx expects, the binary artifact format, a clean storage abstraction — and hardens it for real-world, untrusted CI:
...
- **C — Disable remote caching:** Local cache only. No remote-poisoning risk, but you lose cross-machine and CI cache sharing — fresh CI runners and cold checkouts rebuild from scratch, so large monorepos pay the most.
...
_`remotecache` began from the MIT-licensed [`jase88/nx-cache-server-bun`](https://github.com/jase88/nx-cache-server-bun); the original copyright is preserved in the project's [LICENSE](https://github.com/thilak-rao/remotecache/blob/main/LICENSE)._
