...
| `TOKENS_DB_PATH`            | no       | `./data/nx-cache-server-tokens.sqlite` | SQLite token DB path. Persist this in production. A blank value refuses to start (SQLite would otherwise open a private temporary database); unset it to use the default.                 |
...
| `CACHE_MAX_BYTES`           | no       | —                                      | Positive integer filesystem cache cap in bytes; the sweeper evicts least-recently-used entries until the cache fits.                                                                      |
| `CACHE_TTL_HOURS`           | no       | —                                      | Positive number of hours for the filesystem cache TTL; its millisecond conversion must remain finite. The sweep deletes entries not accessed within the window. Filesystem strategy only. |
| `CACHE_SWEEP_INTERVAL_MS`   | no       | `60000`                                | Positive integer eviction sweep period in milliseconds, up to `2,147,483,647`; used only when a cap or TTL is set.                                                                        |
...
| `BIND_ADDRESS`              | no       | `0.0.0.0`                              | Listen interface. Use `::` for IPv6 / dual-stack. A blank value uses the default.                                                                                                         |
